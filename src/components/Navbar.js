...
const Navbar = () => {
 ...
 const closeMenu = () => {
  ...
  <Link to="/"><img src={Logo} alt="Sunnyside" className="logo" /></Link>
  <Hamburger className='nav-toggle' size={24} toggled={isOpen} toggle={setOpen} color="#FFF" label="Show menu" distance="sm" onToggle={toggled => {
  if (toggled) { setShowLinks(true) 
   ...
   <div class="arrow-up"></div>
      ...
      {links.map((link)=> {
        const {id, text} = link;
        return <li onClick={() => {closeMenu()}} key={id}><Link >{text}</Link></li>
       })}
      <button onClick={() => {closeMenu()}} className="contact">Contact</button>
      </ul>
